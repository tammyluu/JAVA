...
public class PostController {
    private IBlogServcie<PostDTO> postService;
    @Value("IT-Edu")
    private String blogName;
    @Value("it_edu@edublog.edu")
    private String blogContact;

    public PostController(IBlogServcie<PostDTO> postService) {
    ...
    public String addStudent(@Valid @ModelAttribute("post") PostDTO post, BindingResult result) {
        if (result.hasErrors()) {
            return "post-form";
        } else {
            if (post.getId() != null) {
