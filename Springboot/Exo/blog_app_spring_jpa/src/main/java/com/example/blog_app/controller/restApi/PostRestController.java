...
public class PostRestController {
    ...
    public ResponseEntity<String> createCommentValid(@Valid @RequestBody CommentDTO comment, BindingResult result, @PathVariable("postId") UUID id){
        if(result.hasErrors()){
    ...
    public PostDTO updatePost(@PathVariable UUID id, @RequestBody PostDTO updatePost){
        return postService.update(id,updatePost);
    }
    @PutMapping("/updateComment/{id}") // http://localhost:8080/api/v1/blog/updateComment/x
    public ResponseEntity<String> updatePostValid(@PathVariable UUID id, @Valid @RequestBody PostDTO updatePost, BindingResult result){
        if(result.hasErrors()){
            StringBuilder errors = new StringBuilder();
            result.getAllErrors().forEach(objectError -> errors.append(objectError.toString() + " , "));
